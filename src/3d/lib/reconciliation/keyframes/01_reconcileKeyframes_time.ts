import type { type_reconciliation_keyframe } from '../nodes/nodes_reconcile';
import type { type_separatedKeyframes, type_separatedKeyframes_reconciled, type_keyframe_model, type_keyframe_camera } from './types';

/**
 * Represents a keyframe with reconciled absolute start and end times
 */
type type_keyframes_reconciledTime = {
	reconciled: {
		startTime: number;
		endTime: number;
	};
	keyframe: type_reconciliation_keyframe;
};

/**
 * Represents a model keyframe with reconciled absolute start and end times
 */
type type_keyframes_reconciledTime_model = {
	reconciled: {
		startTime: number;
		endTime: number;
	};
 ...
 */
type type_keyframes_reconciledTime_camera = {
	reconciled: {
		startTime: number;
		endTime: number;
	};
 ...
 */
function isAbsoluteKeyframe(kf: type_reconciliation_keyframe): kf is type_reconciliation_keyframe & { time: { type: 'absolute'; value: number } } {
 ...
 */
function isRelativeOrMultipleKeyframe(kf: type_reconciliation_keyframe): kf is type_reconciliation_keyframe & { time: { type: 'relative'; value: any } | { type: 'multiple'; values: any[] } } {
 ...
 */
function createReconciledKeyframe(startTime: number, keyframe: type_reconciliation_keyframe): type_keyframes_reconciledTime {
	return {
		reconciled: {
			startTime,
 ...
 */
function tryResolveKeyframeTime(keyframe: type_reconciliation_keyframe, resolvedMap: Map<string, type_keyframes_reconciledTime>): { resolved: boolean; startTime: number } {
 ...
 */
function validateKeyframes(keyframes: Array<type_reconciliation_keyframe>): Array<type_reconciliation_keyframe> {
...
const reconcileKeyframesTimeInternal = (keyframes: Array<type_reconciliation_keyframe>): Array<type_keyframes_reconciledTime> => {
	...
	const resolvedMap = new Map<string, type_keyframes_reconciledTime>();
	// Array to store the final reconciled keyframes in order
	const reconciledResult: Array<type_keyframes_reconciledTime> = [];
	...
	for (let iteration = 0; iteration < MAX_ITERATIONS && unresolvedKeyframes.length > 0; iteration++) {
		const newlyResolved: Array<type_keyframes_reconciledTime> = [];
 ...
 */
export const reconcileKeyframesTime = (separatedKeyframes: type_separatedKeyframes): type_separatedKeyframes_reconciled => {
	...
	const modelKeyframes: type_keyframes_reconciledTime_model[] = [];
	const cameraKeyframes: type_keyframes_reconciledTime_camera[] = [];

	reconciledAll.forEach((reconciled) => {
		if ('sceneObject' in reconciled.keyframe) {
			modelKeyframes.push(reconciled as type_keyframes_reconciledTime_model);
		} else {
			cameraKeyframes.push(reconciled as type_keyframes_reconciledTime_camera);
		}
	});
