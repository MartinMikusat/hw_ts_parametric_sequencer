import type { type_separatedKeyframes_reconciled2D, type_separatedKeyframes_extended2D, type_keyframe_model2D, type_keyframe_camera2D } from './types';

// Typed versions for separated keyframes
export type type_keyframes_reconciledTimeExtended_model2D = {
	reconciled: {
		startTime: number;
		endTime: number;
	};
...
export type type_keyframes_reconciledTimeExtended_camera2D = {
	reconciled: {
		startTime: number;
		endTime: number;
	};
 ...
 */
export const extendTimeOfKeyframes2D = (separatedKeyframes: type_separatedKeyframes_reconciled2D): type_separatedKeyframes_extended2D => {
	// Process model keyframes
	const modelKeyframes = separatedKeyframes.modelKeyframes.map((kf: { reconciled: { startTime: number; endTime: number }; keyframe: type_keyframe_model2D }) => {
		...
		return {
			...kf,
			extended,
		} as type_keyframes_reconciledTimeExtended_model2D;
	});

	// Process camera keyframes
	const cameraKeyframes = separatedKeyframes.cameraKeyframes.map((kf: { reconciled: { startTime: number; endTime: number }; keyframe: type_keyframe_camera2D }) => {
		const { startTime, endTime } = kf.reconciled;
		const extended = extendSingleKeyframe(startTime, endTime, kf.keyframe.id);
		...
		return {
			...kf,
			extended,
		} as type_keyframes_reconciledTimeExtended_camera2D;
	});
